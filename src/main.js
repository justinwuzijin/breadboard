...
// ---------------------------------------------------------------- state
const state = { parts: [], wires: [], uid: 1, sourceCircuit: null };
/** When true, board mutations from circuit→board import must not clear sourceCircuit. */
let importingCircuit = false;

function touchBoard() {
  if (importingCircuit) return;
  state.sourceCircuit = null;
}
...
function save() {
  const data = {
    ...
    sourceCircuit: state.sourceCircuit || null,
  };
...
function load() {
  ...
  state.sourceCircuit = data.sourceCircuit || null;
}
...
document.getElementById('btn-clear').addEventListener('click', () => {
  ...
  state.parts = [];
  state.wires = [];
  if (!importingCircuit) state.sourceCircuit = null;
  select(null);
  rebuildOcc();
  save();
});
...
function buildFromCircuitToBoard(status) {
  ...
  let source = null;
  try {
    if (typeof cjSim.exportCircuit === 'function') source = cjSim.exportCircuit();
  } catch (_) { /* keep null */ }
  importingCircuit = true;
  try {
  ...
  // Keep the live schematic dump so board→circuit can restore layout/wiring
  // instead of inventing a new auto-layout (lab4 round-trip).
  if (source && typeof status.textContent === 'string' && status.textContent.startsWith('built')) {
    state.sourceCircuit = source;
    saveSoon();
  } else {
    state.sourceCircuit = null;
  }
...
function buildFromBoardToCircuit(status) {
  ...
  try {
    let text = state.sourceCircuit;
    let message = null;
    if (text) {
      message = 'restored original schematic layout';
    } else {
      ({ text, message } = exportBreadboardToText(state));
    }
    openSchematicPanel();
    if (!text) {
